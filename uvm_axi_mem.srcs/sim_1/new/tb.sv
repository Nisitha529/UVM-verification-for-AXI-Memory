    ...
    virtual task run_phase(uvm_phase phase);
        forever begin
            ...
            if (tr.op == rstdut) begin
                reset_dut();
            end else if (tr.op == wrrdfixed) begin
...
class mon extends uvm_monitor;
    ...
    task compare();
        if (err == 0 && rdresp == 0 && wrresp == 0) begin
            `uvm_info("MON", $sformatf("Test Passed err :%0d wrresp :%0d rdresp :%0d ", err, wrresp, rdresp), UVM_MEDIUM);
            err = 0;
        end else begin
            `uvm_info("MON", $sformatf("Test Failed err :%0d wrresp :%0d rdresp :%0d ", err, wrresp, rdresp), UVM_MEDIUM);
            err = 0;
        end
    endtask
...
module tb;
    ...
    initial begin
        `uvm_info("TB_TOP", "Start of the verification.", UVM_NONE);
